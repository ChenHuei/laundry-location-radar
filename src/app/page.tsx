...
export default async function Home({ searchParams }: { searchParams: Promise<{ filter?: string }> }) {
  ...
  return <main className="wrap">
    <div className="top"><div><div className="muted">{businessConfig.districts.join("、")}・Oday 加盟候選</div><h1>洗衣店選址雷達</h1><div className="muted">每日掃描 · 分數與實際開店條件一起檢查</div></div><nav><Link href="/setup">服務設定</Link> · <Link href="/import">匯入物件</Link> · <Link href="/login">管理者</Link></nav></div>
