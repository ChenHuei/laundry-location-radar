...
export function ImportForm({ enabled }: { enabled: boolean }) {
  ...
  return <>
    <form className="panel" onSubmit={async e => {
      ...
      await submit(JSON.stringify([{ source: "manual", sourceId, title: String(data.get("title")), url, address: String(data.get("address")), district: String(data.get("district")), rent: Number(data.get("rent")), areaPing: Number(data.get("areaPing")), firstFloorPing: optionalNumber("firstFloorPing"), lat: optionalNumber("lat"), lng: optionalNumber("lng") }]));
      ...
      <label>行政區<select name="district">{businessConfig.districts.map(district => <option key={district} value={district}>{district}</option>)}</select></label>
      <label>完整地址<input name="address" required maxLength={500} /></label>
